...
namespace Worktastic.Controllers
{
    ...
    public class JobPostingController : Controller
    {
        ...
        public IActionResult CreateEditJob(JobPosting jobPosting, IFormFile file)
        {
            ...
            {
                // Entity Framework query, whereby search and give us that element which we had
                // given above (jobPosting.Id):
                var jobFromDb = _context.JobPostings.SingleOrDefault(x => x.Id == jobPosting.Id);
                if (jobFromDb == null)
                {
                    // Im Falle eines schweren Pogrammierfehlers gib eine 404-Seite aus:
                    return NotFound();
                }
        ...
        public IActionResult DeleteJobPostingById(int id)
        {
            ...
            _context.JobPostings.Remove(jobPostingsFromDb);
            _context.SaveChanges();
